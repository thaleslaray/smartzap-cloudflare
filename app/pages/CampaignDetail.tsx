...
export default function CampaignDetail() {
  ...
  return (
    <div className="space-y-6">
      <Link to="/campaigns" className="text-body text-zinc-500 hover:text-zinc-300">← Campanhas</Link>

      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <h1 className="text-title font-bold tracking-[-0.01em]">{c.name}</h1>
      ...
      <div className="grid grid-cols-[2fr_1fr] gap-4">
        <Card className="p-5">
          <div className="mb-3 flex items-baseline justify-between">
            <p className="text-body text-zinc-400">Progresso ao vivo</p>
        ...
        <div className="flex flex-col justify-between rounded-[--radius-app] border border-primary-500/25 bg-primary-500/[0.06] p-5">
          ...
          <p className="mt-3 text-display font-bold tracking-[-0.02em] text-primary-300">
      ...
      <Card className="overflow-hidden">
        <div className="px-5 py-4">
          <h2 className="text-subtitle font-semibold">Destinatários</h2>
        </div>
        <div className={`grid ${GRID_COLS} gap-4 border-t border-b border-border-subtle px-5 py-2.5 text-caption font-semibold uppercase tracking-wider text-zinc-500`}>
          <span>Contato</span><span>Telefone</span><span>Status</span><span>Erro</span>
        </div>
        {(contacts?.items ?? []).map((r, i) => (
          <div key={i} className={`grid ${GRID_COLS} items-center gap-4 border-b border-border-subtle px-5 py-3`}>
            ...
            {r.error_code ? (
              <span title={String(r.error_detail ?? '')} className="inline-flex w-fit cursor-help items-center gap-2">
                <span className="rounded-md border border-status-failed/30 bg-status-failed/10 px-1.5 py-0.5 font-mono text-caption font-semibold text-status-failed">
                  {String(r.error_code)}
                </span>
              </span>
            ) : (
