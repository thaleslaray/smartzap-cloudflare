...
export default function Knowledge() {
  ...
  return <div className="max-w-5xl space-y-6">
    ...
    <Card className="space-y-3 p-5">
      ...
      <div className="border-t border-zinc-800 pt-4"><p className="mb-2 text-sm font-medium">Ou envie um PDF textual</p><div className="flex flex-col gap-2 sm:flex-row"><input type="file" accept="application/pdf,.pdf" aria-label="Selecionar PDF para a base de conhecimento" onChange={(event) => setFile(event.target.files?.[0] ?? null)} className="text-sm text-zinc-400" /><button type="button" disabled={!file || uploadPdf.isPending} onClick={() => uploadPdf.mutate()} className={btnSecondary}>{uploadPdf.isPending ? 'Extraindo…' : 'Enviar PDF'}</button></div>{uploadPdf.error && <p className="mt-2 text-sm text-status-failed">{uploadPdf.error.message}</p>}</div>
    </Card>
    <Card className="space-y-3 p-5">
