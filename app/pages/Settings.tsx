...
function Field({ f, form, data, onChange }: {
  ...
  return (
    <div>
      <label htmlFor={f.key} className="mb-1 block text-sm text-zinc-400">{f.label}</label>
      <input
        id={f.key}
...
export default function SettingsPage() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        {HEALTH_CHECKS.map((h) => (
          <div key={h.label} className="flex items-center gap-2.5 rounded-[--radius-app] border border-primary-500/30 bg-zinc-900 px-4 py-3.5">
            ...
            <div>
              <p className="text-body font-semibold">{h.label}</p>
              <p className="mt-0.5 text-caption text-zinc-500">{h.detail}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <Card className="p-5">
        <h2 className="text-subtitle font-semibold">Credenciais Meta</h2>
      ...
      <Card className="p-5">
        <h2 className="text-subtitle font-semibold">Webhook</h2>
      ...
      <Card className="p-5">
        <h2 className="text-subtitle font-semibold">Throttle de envio</h2>
      ...
      <Button onClick={() => save.mutate()} disabled={!Object.keys(form).length} loading={save.isPending}>
