...
import { z } from 'zod'
import { validateMetaTemplateComponents } from '../../shared/template-validation'
...
const draftSchema = z.object({ name: z.string().trim().regex(/^[a-z0-9_]{1,512}$/), language: z.string().trim().regex(/^[a-z]{2}_[A-Z]{2}$/), category: z.enum(['MARKETING','UTILITY','AUTHENTICATION']), components: z.array(componentSchema).min(1).max(20) }).strict()
...
export const templatesRoutes = new Hono<{ Bindings: Env }>()
  .get('/', async (c) => {
    ...
    return c.json({ items: [
      ...
      ...drafts.map((draft) => ({ ...draft, synced_at: draft.updated_at, requiresParameters: templateRequiresParameters(draft.components), source: 'draft' as const })),
    ] })
  })
  .post('/sync', async (c) => {
