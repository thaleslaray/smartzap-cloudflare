export function installerHtml(options: { apiBase?: "./" | "../"; version?: string } = {}): string {
  const apiBase = options.apiBase || "./";
  const version = String(options.version || "versão fixa publicada").replace(/[<>&"']/g, "");
  return `<!doctype html>
<html lang="pt-BR">
<head>
  <meta charset="utf-8">
...
<body><main>
  <div class="brand">SmartZap · instalação rápida · ${version}</div>
  <h1>Seu WhatsApp na sua Cloudflare.</h1>
  <p>Você autoriza a conta, confere o plano e instala. Esta rota usa uma versão fixa e não cria um repositório GitHub. Para controlar atualizações e customizações, prefira <a href="../fork/">instalar com seu próprio fork</a>.</p>
  <section class="card" aria-labelledby="title"><h2 id="title">Instalação guiada</h2>
    <div class="step"><span class="number">1</span><div><strong>Autorizar Cloudflare</strong><p class="muted">O provisionador usa OAuth e permissões mínimas. Não pedimos API Token.</p><div class="actions"><a class="button primary" href="${apiBase}oauth/start" id="authorize">Autorizar conta</a><button class="secondary hidden" id="disconnect">Desconectar</button></div></div></div>
...
(() => {
  ...
  const statusAction = (message,label,href) => { status(message,true);const a=document.createElement("a");a.className="button secondary";a.href=href;a.target="_blank";a.rel="noopener noreferrer";a.textContent=label;$("status").append(document.createElement("br"),document.createElement("br"),a); };
  const json = async (url, options={}) => { const response=await fetch(url,{...options,headers:{"Content-Type":"application/json",...(options.headers||{})}});const data=await response.json();if(!response.ok){const error=new Error(data.error||"Falha inesperada");error.code=data.code;throw error}return data; };
  const handlePlanError = (error) => { invalidatePlan();if(error.code==="R2_SUBSCRIPTION_REQUIRED"&&session?.accountId){statusAction(error.message,"Ativar R2 na Cloudflare","https://dash.cloudflare.com/"+session.accountId+"/r2/overview");return}status(error.message,true); };
  async function refresh(){
    try{
      session=await json(API_BASE+"api/session");
  ...
  $("select-account").onclick=async()=>{try{const selected=$("account").value;const accountId=selected==="manual"?$("account-id").value.trim():selected;const found=(session.accounts||[]).find(a=>a.id===accountId);await json(API_BASE+"api/account",{method:"POST",body:JSON.stringify({accountId,accountName:found?.name||"Conta validada"})});status("Conta validada. Agora confira o plano.");session.accountId=accountId;}catch(error){status(error.message,true)}};
