...
- **Fronteiras de evidência:** o reteste remoto ainda é obrigatório antes de staging. Nenhuma mensagem externa foi enviada, nenhuma credencial foi registrada e produção não foi publicada nem alterada. `QA-03` continua `bloqueada` até o canário real Meta; `QA-04` continua `em teste` até canário, calibração humana e soak; `QA-01` permanece `corrigida — reteste pendente` até CI/deploy remoto.
